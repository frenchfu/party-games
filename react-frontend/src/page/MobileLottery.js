...
const customModalStyles = {
  ...
  const showDrowedDrowResult = async (drowData) => {
    ...
    return (<>
        ...
        <img src="/party-games/assets/images/mobiletxt.png" alt="" class="mobiletxt"/>
        <div className={`awardsall02 ${showOverlay ? 'overlay-container' : ''}`}>
            {irxLotRewardsDatas.map((datas, index) => {
                return (
                    <div className="text-center">
                        {datas.map((data, index2) => {
                            return(<RewardDiv mainClass={"awards02"} data={data} doOnClick={doDrowApi} showPaReward={setShowPaReward} showDrowedDrowResult={showDrowedDrowResult} ></RewardDiv>)
                        })}
                    </div>
                )
            })}
